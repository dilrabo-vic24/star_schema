...
def create_dimension_tables(df_raw):
    ...
    dim_subject['subject_id'] = dim_user.apply(lambda _: str(uuid.uuid4()), axis=1)
    dim_subject = dim_subject[['subject', 'subject_id']]

    #2. dim_comm_type
    dim_comm_type = df_raw[['comm_type']].drop_duplicates().reset_index(drop = True)
    dim_comm_type['comm_type_id'] = dim_user.apply(lambda _: str(uuid.uuid4()), axis=1)
    ...
    dim_audio["audio_id"] = dim_user.apply(lambda _: str(uuid.uuid4()), axis=1)
    ...
    dim_video['video_id'] = dim_user.apply(lambda _: str(uuid.uuid4()), axis=1)
    ...
    dim_transcript['transcript_id'] = dim_user.apply(lambda _: str(uuid.uuid4()), axis=1)
    ...
    dim_calendar['calendar_id'] = dim_user.apply(lambda _: str(uuid.uuid4()), axis=1)
